...
import { View, Image, Text, ScrollView, StyleSheet, ImageBackground, Alert } from "react-native";
...
const FormMascota = ({ navigation }) => {
  const { usuario, misMascotas, handlerMascota, handlerMascotasGet } = useContext(MascotaContext);
  ...
  function validateForm() {
    return file !== null && perro.location.latitude !== 0 && perro.petName.length > 0;
  }
  ...
  async function uploadPerro() {
    ...
    let perroId = await crearMascota(perro, token, usuario.notification, usuario._id);
    if (!perroId) {
      ...
      /*  await handlerMascota("crear", mascota); */
      await handlerMascotasGet();
      setOnPressLoading(false);
      Alert.alert("mascota cargada !", "duracion de la publicacion: 6 dias");
    ...
    <ImageBackground source={fondo} style={styles.image} resizeMode="repeat">
      <ScrollView>
        <View style={{ padding: 20 }}>
          ...
          <Button block bordered={false} onPress={pickImage} info style={styles.buttonUnderImage}>
            {image ? (
          ...
          <Button
            ...
            style={[
              ...
              {
                backgroundColor:
                  !validateForm() || topCargas() == true || onPressLoading == true
                    ? "grey"
                    : colores.mainFill,
              },
            ]}
