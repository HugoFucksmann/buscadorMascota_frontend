...
export function ordenarMascotas(mascotas, user) {
  ...
  let mascotaLejos = mascotas.filter((mascota) => distKM(mascota, user).toFixed(1) < 10);
  ...
  mascotaLejos.sort((a, b) =>
    primerosTreinta(a.date) === primerosTreinta(b.date) ? 0 : primerosTreinta(a.date) ? -1 : 1
  );
...
async function adoptar(mid) {
  ...
  return await fetch(`${PROD_URL3}/mascotasAdop/adoptar/${mid}/${JSON.parse(usuario)._id}`, {
    method: "POST",
    headers: {
      Accept: "application/json",
      "Content-Type": "multipart/form-data",
